...
then
	tvar_lastfile="${tvar_lastfile:3:${#tvar_lastfile}-1}"		# Remove leading "../"
	var_newindex=`echo "${tvar_lastfile}" | cut -f 1 -d " "`	# Get index of last entry, by first word in filename
	let "var_newindex++"
